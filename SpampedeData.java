...
class SpampedeData {
	...
	public BoardCell getNextCellInDir() {
		// Note: the current direction of travel can be found in this.currentMode
	...
	public BoardCell getNextCellFromBFS() {
		...
		while (!cellsToSearch.isEmpty()) {
			BoardCell current = cellsToSearch.poll();
			BoardCell[] neigbors = getNeighbors(current);

			for (BoardCell neighbor : neigbors) {
	...
	private BoardCell getFirstCellInPath(BoardCell start) {
		while (start.getParent() != this.getSnakeHead()) {
			start = start.getParent();
		}
		return start; // TODO: Implement getFirstCellInPath(BoardCell)
	}
	...
	public void reverseSnake() {
		LinkedList<BoardCell> reverseSnake = new LinkedList<BoardCell>();
		// Step 1: unlabel the head
		getSnakeHead().becomeBody();

		while (getSnakeTail() != null) {
			// Step 2: reverse the body parts
			BoardCell tail = this.snakeCells.removeFirst();
			reverseSnake.addFirst(tail);
		}

		// Step 3: relabel the head
		this.snakeCells = reverseSnake;
		this.snakeCells.getLast().becomeHead();

		// Step 4: calculate the new direction after reversing!
		if (getSnakeHead().getColumn() == getSnakeNeck().getColumn()) {
			if (getSnakeHead().getRow() > getSnakeNeck().getRow()) {
